...
#### 실측 (2026-09-22) — 실제 회의에서는 실패했다

실제 5인 회의 2.3분(노트북 마이크, 한 방)으로 재 봤다. **안 된다.**

| 임베딩 모델 | `--speakers 5` 결과 |
|---|---|
| campplus (zh+en) | 39·50·27초 → **3명** (그나마 최선) |
| wespeaker resnet293_LM | 한 명이 **95.3%** |
| wespeaker zh_cnceleb_resnet34_LM | 65·48초 → **2명** |
| eres2netv2 (zh-cn) | 한 명이 **86.6%** |

★ 모델을 바꿀수록 더 나빠졌다. pyannote 가 쓰는 WeSpeaker 최상위 모델이
오히려 전원을 한 덩어리로 뭉쳤다. **모델 문제가 아니라 입력 문제**라는 뜻이다.

구간 나누기 자체는 잘 됐다 — `0:00~0:42`, `0:43~1:33`, `1:34~2:08` 로 발화 전환을
또렷하게 찾았고 마지막에 첫 화자로 되돌아온 것도 맞췄다. 못 하는 건 **누가 누구인지**다.

비트레이트는 꽉 찼었다(2.3분 0.39MB). "소리가 녹음됐다" 와 "목소리가 구별된다" 는
다른 문제다. 한 방에 마이크 하나면 멀리 앉은 사람이 작고, 음색이 뭉개지고, 말이 겹친다.

**다음에 할 일은 모델 교체가 아니라 마이크 교체다.** 회의용 USB 원형 마이크로
다시 녹음해 같은 스크립트로 재 보면 된다. 그래도 안 되면 한국어 특화 상용
엔진(CLOVA Speech)을 보는 순서다.
