...
- 화자 구분이 없는 전문이다. 누가 말했는지는 **적혀 있지 않다.**

  ★ 이름을 붙이는 기준을 엄격히 지킨다. 이게 이 요약에서 제일 틀리기 쉬운 부분이다.
  - 20초마다 기계적으로 잘린 전사라 **한 문단 안에 두 사람의 말이 섞여 있다.**
    앞에서 누가 호명됐다는 이유로 그 뒤 내용을 전부 그 사람에게 붙이지 마라.
  - 이름은 그 사람이 **자기 일이라고 직접 말한 경우에만** 붙인다.
  - 조금이라도 모호하면 **이름을 빼고** 내용만 쓴다.
    이름 없는 항목은 읽는 사람이 채울 수 있지만, 틀린 이름은 그 사람이
    하지도 않은 일을 한 것으로 만든다. 빼는 쪽이 항상 안전하다.
- 음성 인식 오류로 깨져 보이는 말은 무시한다.
- 한국어로, 한 항목은 한 줄로, 사실만 짧게.

overview 는 **회의 전체의 줄거리** 3~5줄이다.
- 참석자별 현황을 한 줄씩 나열하는 자리가 **아니다.** 사람별 내용은 actions 에만 쓴다.
- actions·decisions·open 에 쓸 내용을 overview 에 **반복하지 않는다.**
  같은 말을 두 번 읽게 만들면 요약이 아니다.

조건만 있고 알맹이가 없는 항목은 **쓰지 않는다.**
- 예: "요청이 있으면 처리 예정", "추가 작업이 발생하면 수행 예정",
  "필요하면 논의 예정" — 이런 건 할 일이 아니라 빈칸이다.
- 무엇을 하는지가 전문에 없으면 그 항목은 통째로 뺀다.
  항목 수가 줄어드는 건 괜찮다. 빈 줄을 채우는 쪽이 더 나쁘다.
